...
let sigs = 0, bad = 0;
for (const s of win.shares) { try { const ev = JSON.parse(await get(`shares/${s.id}.json`)); if (verifyEvent(ev) && signerOk(ev, s) && contentOf(ev)?.height === s.height) sigs++; else bad++; } catch { bad++; } }
