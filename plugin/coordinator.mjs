...
export class Coordinator {
  constructor({ k, pow, hash, rpc, key, params, dataDir, log = console.log }) {
    ...
    this.assignments = new Map(); this.assignmentsByMaster = new Map(); this.lastRetarget = 0; this.recentReceipts = [];
  ...
  async share(conn, ev) {
    ...
    if (r.splitId === 'solo') { // a receipt: verified, kept, never in the window
      ...
      this.seen.add(r.hash); this.stats.receipts++; this.recentReceipts.push({ master: r.master, at: rec.at }); if (this.recentReceipts.length > 5000) this.recentReceipts.splice(0, 1000);
